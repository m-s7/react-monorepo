...
export interface BreadcrumbsRoute<K extends string = string> extends RouteObject {
...
const useRouterBreadcrumbs = (routes?: BreadcrumbsRoute[], options?: Options): BreadcrumbData[] => getBreadcrumbs(routes || [], useLocation(), options)

const getBreadcrumbs = (routes: BreadcrumbsRoute[], location: Location, options?: Options): BreadcrumbData[] => {
...
const getBreadcrumbMatch = (path: string, location: Location, pathName: string, branches: BreadcrumbsRoute[], options?: Options): typeof NO_BREADCRUMB | BreadcrumbData => {
...
const render = (breadcrumb: BreadcrumbComponentType | string, match: BreadcrumbMatch, location: Location, stringReturn?: boolean, props?: { [x: string]: unknown }): BreadcrumbData => {
    ...
    return {
        ...componentProps,
        breadcrumb:
            typeof breadcrumb === 'string' ? (stringReturn ? breadcrumb : (createElement('span', { key: componentProps.key }, breadcrumb))) : (<Component {...componentProps} />),
    }
}
